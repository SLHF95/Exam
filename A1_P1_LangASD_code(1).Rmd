...
```{r}
setwd("C:/Users/slhf9/Dropbox/0Aarhus Universitet/Experimental Methods III/Assignment 1/ass1")
```
...
*"\\d" means regular expression + digits*
*str_extract(where you want to extract, what you want to extract)* 
...
*gsub(what you want to replace, what you want to replace it with, where)*
...
dfAll = merge(d1and2, d3, by = c("SUBJ1", "VISIT"))
```
The number of observations changes - df1 used to be 372 observations, but the new dataframe is only 352 observations. 
...
```{r}
dfV1 = subset(dfAll, dfAll$VISIT == 1) 
...
dfAll = merge(dfAll, dfV1, by ="SUBJ1")
dfAll = subset(dfAll, select = -c(ADOS, MullenRaw, ExpressiveLangRaw))
...
```{r}
str(dfAll)
dfAll$SUBJ1 = as.factor(dfAll$SUBJ1)
levels(dfAll$SUBJ1) = 1:length(levels(dfAll$SUBJ1))
```
...
```{r}
dfAll$VISIT = as.numeric(dfAll$VISIT)
```
...
dfAll$Gender = as.factor(dfAll$Gender)
dfAll$Gender = plyr::revalue(dfAll$Gender, c("1" = "M", "2" = "F"))

dfAll$Diagnosis = plyr::revalue(dfAll$Diagnosis, c("A" = "ASD", "B" = "TD"))
...
```{r}
write.csv(dfAll, file = "ass1_LangASD.csv")
```
...
dfAll %>% group_by(SUBJ1) %>%
  summarise(mlu = mean(CHI_MLU, na.rm = TRUE)) %>%
  filter(mlu > 2.7)
...
```{r}
filter(dfAll, CHI_MLU < 1.5 & VISIT == 1)
```
...
```{r}
select(dfAll, SUBJ1, Diagnosis, CHI_MLU, tokens_CHI) %>%
  filter(Diagnosis == "ASD")
```

2. What happens if you include the name of a variable multiple times in a select() call?
```{r}
select(dfAll, SUBJ1, Diagnosis, CHI_MLU, CHI_MLU, tokens_CHI) %>%
  filter(Diagnosis == "ASD")
```
No difference?  
...
dfAll = mutate(dfAll, GrandAvg = mean(tokens_CHI))
...
ChiAvg_AcrossVisits = dfAll %>%
  group_by(SUBJ1) %>%
  summarise(ChiAvgWord = mean(tokens_CHI))

dfAll = merge(dfAll, ChiAvg_AcrossVisits)
