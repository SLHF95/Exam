...
N.B. Question to be answered via email to Celine: 
...
```{r}
pitch = read.table("Folderr/Study1D0S101T3_f0.txt", header = TRUE)
```
...
T = trial
```{r}
files <- list.files(path = paste(locpath, "/Folderr", sep = "")) 
                      #"C:/Users/Bruger/Dropbox/0Aarhus Universitet/Experimental Methods III/Assignment 3/ass3/Folderr"

setwd(paste(locpath, "/Folderr", sep = ""))
  #"C:/Users/Bruger/Dropbox/0Aarhus Universitet/Experimental Methods III/Assignment 3/ass3/Folderr")
...
ENTRmodel1 = lm(ENTR ~ 1 + diagnosis + study, data = df)
summary(ENTRmodel1)
#not significant

LAMmodel1 = lm(LAM ~ 1 + diagnosis + study, data = df)
summary(LAMmodel1)
#not significant

TTmodel1 = lm(TT ~ 1 + diagnosis + study, data = df)
summary(TTmodel1)
#not significant 
...
```{r}
PRmodel2 = lm(PR ~ 1 + diagnosis * study, data = df)
summary(PRmodel2)
#not significant

DETmodel2 = lm(DET ~ 1 + diagnosis * study, data = df)
summary(DETmodel2)
#significant

NRLINEmodel2 = lm(NRLINE ~ 1 + diagnosis * study, data = df)
summary(NRLINEmodel2)
#not significant 

maxLmodel2 = lm(maxL ~ 1 + diagnosis * study, data = df)
summary(maxLmodel2)
#not significant

Lmodel2 = lm(L ~ 1 + diagnosis * study, data = df)
summary(Lmodel2)
#not significant

ENTRmodel2 = lm(ENTR ~ 1 + diagnosis * study, data = df)
summary(ENTRmodel2)
#not significant

LAMmodel2 = lm(LAM ~ 1 + diagnosis * study, data = df)
summary(LAMmodel2)
#nsignificant

TTmodel2 = lm(TT ~ 1 + diagnosis * study, data = df)
summary(TTmodel2)
#not significant 

meanmodel2 = lm(mean ~ 1 + diagnosis * study, data = df)
summary(meanmodel2)
#

sdmodel2 = lm(sd ~ 1 + diagnosis * study, data = df)
summary(sdmodel2)
#s

rangemodel2 = lm(range ~ 1 + diagnosis * study, data = df)
summary(rangemodel2)
#significant
```
